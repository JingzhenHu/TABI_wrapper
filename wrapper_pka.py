...
def site_site_interaction(s1,s2,PBDID,usrdata_map,out_path):
    ...
    # all_mute with s1 and s2 on only
...
def main(PBDID):
    ...
    while PH < 20:
        ...
        obs_status_eng.write(' '.join(['number of energies < 1e-15', str(count1)]))
        obs_status_eng.write('\n')
        obs_status_eng.write(' '.join(['1e-15 <= number of energies < 1e-5', str(count2)]))
        obs_status_eng.write('\n')        
        obs_status_eng.write(' '.join(['1e-5 <= number of energies', str(len(status_Energy)-count1-count2)]))
        obs_status_eng.write('\n')
        err_tol = 0.05
